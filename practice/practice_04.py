...
import numpy as np
from load_utils import load_dates, load_codes, load_one_stock, load_matrix, load_column, load_close_flat
"""
...
arr6 = np.array(list6)
arr6 = np.round(arr6).astype('int64')
print(f"arr6 : {arr6}")
...
"""
standard = load_one_stock(0)
dates = load_dates()
max_idx = standard.argmax()
min_idx = standard.argmin()
print(f"{standard[max_idx]:,}, ({dates[max_idx]}), ({standard[min_idx]:,}, ({dates[min_idx]})")
